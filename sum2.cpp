...
struct bigint {
    ...
    const string getString() const {
        ...
        if (sign == -1)
            val += string("-");
        int t = (a.empty() ? 0 : a.back());
        val += to_string(t);
        for (int i = (int) a.size() - 2; i >= 0; --i) {
            string v = to_string(a[i]);
            int len = v.length();
            for (int i = 0; i < base_digits - len; i++) {
                v = string("0") + v;
            }
            val += v;
        }
...
int sum(int argc, char* argv[]) {
    ...
    //BigInteger max;
    if (argc == 3) {
        //max.setNumber(argv[2]);
        max.read(argv[2]);
    } else {
        //max.setNumber("1000");
        max.read("1000");
    }
    //    BigInteger nTasks(size);
    //    BigInteger pSize = max / nTasks;
    //    BigInteger c(rank);
    //    BigInteger start = pSize * c;
    // c++;
    bigint nTasks(size);
    bigint pSize = max / nTasks;
    bigint c(rank);
    bigint start = pSize * c;
    c = c + bigint(1);
    ...
    for (bigint i = start; i <= end; i = i + bigint(1)) {
        sum += i;
    }
    ...
    if (rank == 0) {
       ...
       for (int i = 1; i < size; i++) {
            ...
            string partial_sum(value);
            //BigInteger pSum(partial_sum);
            bigint pSum(partial_sum);
            sum += pSum;
        }
        ...
        //string num = sum.getNumber();
        string num = sum.getString();
        int len = num.length();
        MPI_Send(&len, 1, MPI_INT, 0, 1, MPI_COMM_WORLD);
        MPI_Send(&num[0], len + 1, MPI_CHAR, 0, 2, MPI_COMM_WORLD);
    }
    MPI::Finalize();
    return 0;
}