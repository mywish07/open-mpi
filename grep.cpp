...
int Grep::grep(int argc, char* argv[]) {
    ...
    long long int *pLineStartIndex; // keep track of which lines start where. 
    char *pszFileBuffer; 
    int nTotalLines = 0;
    ...
    while (1) {
        ...
        if (cont == 0) 
            break;
        if (rank == 0) {
            char* ptr = strchr(&pszFileBuffer[0], 10); // find a new line character
            while (ptr != NULL) {
                nTotalLines++;
...
void Grep::print(char* str, int start, int len) {
    cout << "@";
    for (int i = 0; i < len; i++) {
        if (str[start + i] == '\r')
            cout << "EOL";
...
char* Grep::readFile(FILE* file, size_t fileSize) {
    ...
    char* str = new char[readsize];
    fseek(file, lastPosition, SEEK_SET); // read file from last position
    fread(&str[remainLen], 1, readsize, file);
